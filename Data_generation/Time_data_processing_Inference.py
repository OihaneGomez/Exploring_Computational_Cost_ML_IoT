...
def gather_one(class_dict, features):
    ...
    for c in class_dict.keys():
        f = glob.glob(root + c + '/*') #Get all files 
        f_final=f[0:1]
        d = pd.DataFrame(reformat(f_final, cls=c, features=features)) #Pandas dataframe for reformat funtion features
        df.append(d)
    return pd.concat(df)
...
with open('Inference_processing_times_'+str(number_components)+'Comp_'+str(best_features_number)+'Features.txt', 'w') as f:
    f.write(cap.stdout)      