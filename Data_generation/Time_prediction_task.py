...
root_test = './HMP_Dataset_Test/' 
def gather_test(class_dict):
    df = []
    for c in class_dict.keys():
        f = glob.glob(root_test + c + '/*') #Get all files
        d = pd.DataFrame(reformat(f, cls=c)) #Pandas dataframe for reformat funtion features
        df.append(d)
    return pd.concat(df)

def gather_one(class_dict):
    df = []
    for c in class_dict.keys():
        f = glob.glob(root + c + '/*') #Get all files 
        f_final=f[0:1]
        d = pd.DataFrame(reformat(f_final, cls=c)) #Pandas dataframe for reformat funtion features
        df.append(d)
    return pd.concat(df)
...
#Process all the dataset for creating the model
...
wrist_df_test = gather_one(wrist_class)
#Extact number of the class (feature Y)
wrist_Y_test = np.asarray(wrist_df_test.Y)
...
#,random_state=0
    #for train_index, test_index in kf.split(wrist_X):
...
for i in range (0,(best_features_number)): 
    ...
    best_features_num=i+1
    print( "Features number = "+ str(best_features_num))

    select_feature = SelectKBest(chi2, k=best_features_num)

    #Create an array with the most representative features
    wrist_X_new = select_feature.fit_transform(wrist_X, wrist_Y)
    wrist_X_new_test = select_feature.fit_transform(wrist_X_test, wrist_Y_test)
    ...
    for item, item_name in zip(alg_array, alg_array_names): 
        ...
        #Fit the model for every algorithm
        classifier.fit(wrist_X_new, wrist_Y)

        #Calculate fitting time 
        timer = get_ipython().run_line_magic('timeit', '-n 3 -r 100 -o classifier.predict(wrist_X_new_test)')
...
with open('Prediction_task_times_'+str(number_components)+'Comp_'+str(best_features_number)+'Features_mean.txt', 'w') as f:
    f.write(cap.stdout)    

#Save data in .txt file, std values
get_ipython().run_cell_magic('capture', 'cap ', 'print(array_dict_std)')
with open('Prediction_task_times_'+str(number_components)+'Comp_'+str(best_features_number)+'Features_std.txt', 'w') as f:
    f.write(cap.stdout) 